...
class DETR3DAssoTrackingHead(nn.Module):
    ...
    def __init__(self,
        ...
        """
        super(DETR3DAssoTrackingHead, self).__init__()

        if 'code_size' in kwargs:
            self.code_size = kwargs['code_size']
        else:
            # adds 1 for turn rate
            self.code_size = [6, 2, 2, 1]
    ...
    def _init_asso_heads(self):
        ...
        fc_asso = nn.Sequential(*asso_branch)

        def _get_clones(module, N):
            return nn.ModuleList([copy.deepcopy(module) for i in range(N)])

        num_pred = self.transformer.decoder.num_layers
        self.asso_branches = _get_clones(fc_asso, num_pred)
    ...
    def forward(self, mlvl_feats, radar_feats,
                query_embeds, ref_points, ref_size, img_metas, track_mask, **kwargs):
        """Forward function.
        Args:
        ...
        # take 0th cam image (assumes all of same size)
        input_img_h, input_img_w = img_metas[0]['input_shape'][0]
        ...
        for lvl in range(hs.shape[0]):
            ...
            # TODO (shuxiao): `reg_branches` are already called in transformer decoder, outputs are 
            # already included in `reference` and `ref_size_base`, the predicted offsets are
            # used again in xywlzh to update box parameters
            ...
            bbox_pred = torch.cat(
                [xywlzh, direction_pred, velo_pred], dim=2)
        ...
        return outputs_classes, outputs_coords, output_affinities, inter_edge_index, \
